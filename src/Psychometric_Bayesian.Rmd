...
The model object used in these analyses is stored in 'results/Psychometric_Bayesian_model_object.rds'. To use it, load it into an object named: 'TW_col.dta1'.
...
```{r}
cd.long <- read_delim('../data/colour_discriminate_long_format.txt', delim="\t")
```
...
Marginal effects at the mean is used to investigate certain parameters at the mean values of the remaining parameters. 
