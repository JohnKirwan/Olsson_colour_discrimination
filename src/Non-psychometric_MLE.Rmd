...
Rule 2, keep target
```{r}
lme.m1 <- update(lme.m, .~. - Colour.difference - 
							(1 + Colour.difference | chick) - 
							(1 + Colour.difference | batch) 
							+ (1 | chick) + 
							(1 | batch) 
...
library(lsmeans)
ls.comps <- lsmeans(lme.m, list(pairwise ~ target))
summary(ls.comps)[2]
```
...
```{r}
formula(lme.m)#final model, with only "significant terms"
```
...
#   .   sex
```{r}
anova(lme.m, update(lme.m, .~. +sex))[ifo]
```
#   .   background
```{r}
anova(lme.m, update(lme.m, .~. +background))[ifo]
```
#   .   same:sex
```{r}
anova(lme.m, update(lme.m, .~. +target*sex))[ifo]
```
#   .   Colour.difference:target
```{r}
anova(lme.m, update(lme.m, .~. +Colour.difference*target))[ifo]
```
#   .   background:target
```{r}
anova(lme.m, update(lme.m, .~. +background*target))[ifo]
```
#   .   Colour.difference:background
```{r}
anova(lme.m, update(lme.m, .~. +Colour.difference*background))[ifo]
```
#   .   background:sex
```{r}
anova(lme.m, update(lme.m, .~. +background*sex))[ifo]
```
#   .   Colour.difference:sex
```{r}
anova(lme.m, update(lme.m, .~. +Colour.difference*sex))[ifo]
```
#   .   background:target:sex
```{r}
anova(lme.m, update(lme.m, .~. +background*target*sex)
    )[2,ifo]
```
#   .   Colour.difference:background:target
```{r}
unlist(
  anova(lme.m, update(lme.m, .~. +Colour.difference*background*target)
        )[2,ifo]
)
```
...
```{r}
unlist(
  anova(lme.m, update(lme.m, .~. +Colour.difference*background*sex)
        )[2,ifo]
)
```
...
```{r}
unlist(
  anova(lme.m, update(lme.m, .~. +Colour.difference*target*sex))[2,ifo]
)
```
#   .   Colour.difference:background:same:sex
```{r}
unlist(
  anova(lme.m, update(lme.m, .~. +Colour.difference*background*target*sex))[2,ifo]
)
```
